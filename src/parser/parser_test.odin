...
test_parser :: proc(t: ^testing.T) {
	...
	testing.expect(
		t,
		len(program.stmts) > 0,
		"expected more than one global statement",
	)

	fn, ok := program.stmts[0].(^ast.FunctionStatement)
	testing.expect(t, ok, "expected function")
	testing.expect(t, len(fn.args) == 0, "expected 0 args")
	testing.expect(
		t,
		len(fn.body.stmts) > 0,
		"expected to have function body statements",
	)

	{
		assign, ok1 := fn.body.stmts[0].(^ast.AssignStatement)
		testing.expect(t, ok1, "expected first statement to be an assignment")

		infix, ok2 := assign.value.(^ast.InfixExpr)
		testing.expect(t, ok2, "expected expression to be infix")

		left, ok3 := infix.left.(^ast.IntLiteral)
		testing.expect(t, ok3, "expected int literal for left")
		testing.expectf(
			t,
			left.value == 34,
			"expected left to be 34, got %d",
			left.value,
		)

		right, ok4 := infix.right.(^ast.IntLiteral)
		testing.expect(t, ok4, "expected int literal for right")
		testing.expectf(
			t,
			right.value == 35,
			"expected right to be 35, got %d",
			right.value,
		)
	}
	...
	testing.expect(
		t,
		len(fn.body.stmts) > 2,
		"expected to have at least 2 statements",
	)

	expr_stmt, ok5 := fn.body.stmts[1].(^ast.ExprStatement)
	testing.expect(t, ok5, "expected expression statement")

	call_expr, ok6 := expr_stmt.value.(^ast.CallExpr)
	...
	fmt, ok7 := call_expr.args[0].(^ast.StringLiteral)
	testing.expect(t, ok7, "expected string literal for first call expr arg")
	testing.expectf(
		t,
		fmt.value == "%d\\n",
		"expected printf format string, got %s",
		fmt.value,
	)

	ident, ok8 := call_expr.args[1].(^ast.Identifier)
	testing.expect(t, ok8, "expected identifier for second call expr arg")
	testing.expectf(
		t,
		ident.value == "foo",
		"expected ident foo, got %s",
		ident.value,
	)
}
